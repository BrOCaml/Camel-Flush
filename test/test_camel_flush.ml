...
(** Test Plan: Functions that handle card, deck, player, and combo logic were
    ...
    cards, creating combos and checking valid combos and higher combos. Test
    cases were mainly developed to ensure game functionality of each component.
    For each component, we test it's functionality on a micro level, e.g.
    checking the higher combo of two combos. After ensuring that each comonent
    is working properly, we manually test on the macro level (running the game)
    that everything is operating as it should. By this, we can ensure
    correctness of the entire game functionality. *)
